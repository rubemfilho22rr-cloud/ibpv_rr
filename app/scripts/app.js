...
function printReport(){
  const source=document.querySelector('#preview-content .report-document');
  if(!source){buildPreview();setTimeout(printReport,120);return;}
  const printable=source.cloneNode(true);
  const logo=printable.querySelector('.report-brand-logo');if(logo&&window.IBPV_LOGO_DATA)logo.src=window.IBPV_LOGO_DATA;
  const cssUrl=new URL('styles/print-report.css',window.location.href).href;
  const printWindow=window.open('','_blank','width=980,height=900');
  if(!printWindow){alert('Permita a abertura de janelas para gerar o PDF.');return;}
  printWindow.document.open();
  printWindow.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Relatório Financeiro — ${escapeHtml(formatReportPeriod())}</title><link rel="stylesheet" href="${cssUrl}"></head><body>${printable.outerHTML}<script>window.addEventListener('load',()=>setTimeout(()=>window.print(),250));<\/script></body></html>`);
